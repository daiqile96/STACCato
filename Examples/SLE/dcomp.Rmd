...
cov.mat <- read.table(file.path(data.dir, 'filter_subject_info.txt'), 
...
pdf(file.path(out.dir, 'r_lr.pdf'), 
    height = 4, width = 8)
plot(r2$p)
dev.off()

pdf(file.path(out.dir, 'r_sender.pdf'), 
    height = 4, width = 8)
plot(r3$p)
dev.off()

pdf(file.path(out.dir, 'r_receiver.pdf'), 
    height = 4, width = 8)
plot(r4$p)
dev.off()
```
...
```{r}
plot_sender_receiver(dcomp_res = staccato_all_cov,
...
gridExtra::arrangeGrob(grobs = list(p_sender,
                                    p_receiver), 
                               ncol = 2)
```
