...
plot_cell_score = function(mode, dcomp_res, n, 
                           cell_level, cell_label){
...
get_eff_sig = function(eff_pval_df, eff_cutoff, pval_cutoff){
  ...
  eff_pval_df = eff_pval_df %>% 
    filter(pval_cutoff <= 5e-02 & abs(effect) >= eff_cutoff) %>% 
...
get_contribution = function(comp, mode, cov, dcomp_res){
  ...
  if (mode == 'cov'){
    ...
    U_sub = ttm(C_ts_sub, cov.ind[, -comp], m = 1)
    ...
    U_sub = ttm(C_ts_sub, cov.ind, m = 1)
...
get_contribution_by_mode = function(mode, cov, dcomp_res){
  
  r_mode = ncol(dcomp_res$W[[mode]])
  
  contr = sapply(1:r_mode, 
                 get_contribution, 
                 mode = mode, 
                 cov = cov, 
                 dcomp_res = dcomp_res)
  
  contr_df = data.frame(var = factor(paste0('Factor', 1:r_mode)),
                        mse = contr)
...
staccato = function(tsr,
  ...
  dimnames(C_ts@data) <- list(colnames(cov.ind),
                         lr.names,
                         sender.names,
                         receiver.names)
...
##### bootstrap ########
boot = function(idx, dcomp_res, cov){
...
boot_p = function(n_boot, dcomp_res, cov, n_thread,
  ...
  dimnames(boot.p) = list(colnames(cov),
                          lr.names,
                          sender.names,
                          receiver.names)
